 ...
 *@Method설명 : 페이지 번호를 받아와 해당하는 리스트를 뽑아 qna_board.jsp로 보내준다.
 */
 ...
 *@Method설명 : 제목,내용 등 적은 것들을 컨트롤러로 보내 세션에 남아있는 회원의 이름, 번호를 QnAVO에 set하여 
 *						등록하는 서비스로 전송한다. 그리고 저장된 게시판 번호를 상세글보기로 보내준다.
 */
 ...
 *@Method설명 : 게시물 번호를 통해 게시물이 조회된 적이 있는지 쿠키를 통해 확인을 한다. 
 *						조회된 적이 있다면 조회수를 높이고 그렇지 않다면 조회수를 그대로 둔다. 
 *						(쿠키 유무) 그리고 상세글보기로 qna를 넘겨준다
 */
 ...
 *@Method설명 : 수정할 글을 서비스의 update를 이용해 수정하고 글번호를 상세 글을 볼 수 있는 컨트롤러로 전송한다.
 */
 ...
 *@Method설명 :삭제할 글 번호를 받아와  delete 서비스를 통해 삭제하고 pageNo와 함께 qna_board로 이동한다
 */
 ...
 *@Method설명 :답글 폼으로 이동하기 위해 qna번호를 통해 qna글을 가져온다.
 */
 ...
 *@param QnABoardVO : qna_reply_view에서 가져온 글제목과 관리자가 입력한 글내용과 시간을 가져온다.

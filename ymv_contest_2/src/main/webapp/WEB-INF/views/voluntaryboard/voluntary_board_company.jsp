			...
			<tbody>
				<c:forEach var="bvo" items="${requestScope.lvo.list}">
					<tr>
						...
						<td>
							<a href="${initParam.root}voluntary_show_content.ymv?memberNo=${sessionScope.mvo.memberNo }&recruitNo=${bvo.recruitNo}&mojib=${bvo.mojib }&noApplicate=yes"> ${bvo.title }</a>
						</td>
						</c:when>
						<c:otherwise>
